...
class AABB {
	...
	contains(aabb) {
		...
		return (
			l1.x <= l2.x && u1.x >= u2.x && l1.y <= l2.y && u1.y >= u2.y && l1.z <= l2.z && u1.z >= u2.z
		);
	}
...
class Body extends EventTarget {
	...
	constructor(options) {
		...
		this.collisionResponse =
			typeof options.collisionResponse === 'boolean' ? options.collisionResponse : true;
		this.position = new Vec3();
...
class Ray {
	...
	_intersectSphere(sphere, quat, position, body, reportedShape) {
		...
		const c =
			(from.x - position.x) ** 2 + (from.y - position.y) ** 2 + (from.z - position.z) ** 2 - r ** 2;
...
class RigidVehicle {
	...
	addWheel(options) {
		...
		const position =
			typeof options.position !== 'undefined' ? options.position.clone() : new Vec3(); // Set position locally to the chassis
	...
	update() {
		...
		for (let i = 0; i !== N; i++) {
			...
			for (let j = 0; j !== numNeighbors; j++) {
				...
				u.scale(
					(1.0 / (0.0001 + this.densities[i] * this.densities[j])) * this.viscosity * neighbor.mass,
					u
				);
				nabla = this.nablaw(r);
	...
	constructor(radiusTop, radiusBottom, height, numSegments) {
		...
		for (let i = 0; i < N; i++) {
			...
			if (i < N - 1) {
				// Bottom
				vertices.push(
					new Vec3(-radiusBottom * sin(theta), -height * 0.5, radiusBottom * cos(theta))
				);
...
function barycentricWeights(x, y, ax, ay, bx, by, cx, cy, result) {
	result.x =
		((by - cy) * (x - cx) + (cx - bx) * (y - cy)) / ((by - cy) * (ax - cx) + (cx - bx) * (ay - cy));
	result.y =
		((cy - ay) * (x - cx) + (ax - cx) * (y - cy)) / ((by - cy) * (ax - cx) + (cx - bx) * (ay - cy));
			...
			for (let i = 0; i < bi.shapes.length; i++) {
				...
				for (let j = 0; j < bj.shapes.length; j++) {
					...
					this.currentContactMaterial =
						shapeContactMaterial || bodyContactMaterial || world.defaultContactMaterial; // Get contacts
